...
public class ToggleCommands {
    ...
    @Command(aliases = {"stopfire"}, usage = "[<мир>]",
            desc = "Временно отключает все способы распространения огня", max = 1)
    ...
    public void stopFire(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!wcfg.fireSpreadDisableToggle) {
            worldGuard.getPlatform().broadcastNotification(
                    LabelFormat.wrap("Распространение огня было глобально приостановлено для '" + world.getName() + "' by "
                    + sender.getDisplayName() + "."));
        } else {
            sender.print("Распространение огня уже было глобально остановлено.");
        }
    ...
    @Command(aliases = {"allowfire"}, usage = "[<world>]",
            desc = "Позволяет всему огню временно распространяться", max = 1)
    @CommandPermissions({"worldguard.fire-toggle.stop"})
    public void allowFire(CommandContext args, Actor sender) throws CommandException {
        ...
        if (wcfg.fireSpreadDisableToggle) {
            ...
            sender.print("Распространение огня уже было разрешено по всему миру.");
        }
    ...
    @Command(aliases = {"halt-activity", "stoplag", "haltactivity"}, usage = "[confirm]",
            desc = "Пытается прекратить как можно больше активности, чтобы остановить лаги", flags = "cis", max = 1)
    ...
    public void stopLag(CommandContext args, Actor sender) throws CommandException {
        ...
        if (args.hasFlag('i')) {
            if (configManager.activityHaltToggle) {
                 sender.print("ЛЮБАЯ интенсивная активность сервера запрещена.");
            } else {
                 sender.print("ЛЮБАЯ интенсивная активность сервера разрешена.");
            }
        } else {
            ...
            if (activityHaltToggle && (args.argsLength() == 0 || !args.getString(0).equalsIgnoreCase("confirm"))) {
                ...
                TextComponent message = TextComponent.builder("")
                        .append(ErrorFormat.wrap("Эта команда выполнит "))
                        .append(ErrorFormat.wrap("ПЕРМАНЕНТНОЕ")
                                .decoration(TextDecoration.BOLD, TextDecoration.State.TRUE))
                        .append(ErrorFormat.wrap(" уничтожение ВСЕХ животных во ВСЕХ загруженных фрагментах во ВСЕХ загруженных мирах. "))
                        ...
                        .append(ErrorFormat.wrap(" или напишите "))
            ...
            if (activityHaltToggle) {
                ...
                if (!args.hasFlag('s')) {
                    worldGuard.getPlatform().broadcastNotification(LabelFormat.wrap("ВСЯ интенсивная активность сервера приостановлена игроком " + sender.getDisplayName() + "."));
                } else {
                    sender.print("(Silent) ВСЯ интенсивная активность сервера приостановлена игроком " + sender.getDisplayName() + ".");
                }
                ...
                if (!args.hasFlag('s')) {
                    worldGuard.getPlatform().broadcastNotification(LabelFormat.wrap("Теперь разрешена вся интенсивная работа сервера."));
                    
                    if (!(sender instanceof LocalPlayer)) {
                        sender.print("Теперь разрешена вся интенсивная работа сервера.");
                    }
                } else {
                    sender.print("(Silent) Теперь разрешена вся интенсивная работа сервера.");
                }
            }
        }
    }
