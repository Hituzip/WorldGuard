...
class FlagHelperBox extends PaginationBox {
        ...
        if (flag.usesMembershipAsDefault()) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который по умолчанию разрешает для участников и запрещает для сторонних игроков"))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
        if (flag == Flags.PASSTHROUGH) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который переопределяет проверки сборки. (Не связано с движением!)"))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
    ...
    private <V> void appendValueChoices(TextComponent.Builder builder, Flag<V> flag, Iterator<V> choices, @Nullable String suggestChoice) {
        ...
        while (choices.hasNext()) {
            ...
            if (maySet) {
                if (isExplicitSet) {
                    hoverTexts.add(TextComponent.of("Нажмите, чтобы убрать", TextColor.GOLD));
                } else if (DANGER_ZONE.contains(flag) && !(ProtectedRegion.GLOBAL_REGION.equals(region.getId()) && flag == Flags.PASSTHROUGH)) {
                    hoverTexts.add(TextComponent.of("Установка этого флага может иметь неожиданные последствия.", TextColor.RED)
                            ...
                            .append(TextComponent.newline())
                            .append(TextComponent.of("(Подсказка: вам не нужно устанавливать это для защиты региона!)"))));
                } else {
    ...
    private void appendLocationFlagValue(TextComponent.Builder builder, LocationFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "неустановлено местоположение", null);
            } else {
                appendValueText(builder, flag, defVal.toString(), TextComponent.of("Стандартное значение:")
    ...
    private <V extends Number> void appendNumericFlagValue(TextComponent.Builder builder, NumberFlag<V> flag) {
        ...
        appendValueChoices(builder, flag, (Iterator<V>) choices.iterator(), choices.isEmpty() ? "неустановленный номер" : "[custom]");
    }
    ...
    private void appendStringFlagValue(TextComponent.Builder builder, StringFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "неустановленная строка", null);
            } else {
                ...
                appendValueText(builder, flag, display, TextComponent.of("Стандартное значение:")
