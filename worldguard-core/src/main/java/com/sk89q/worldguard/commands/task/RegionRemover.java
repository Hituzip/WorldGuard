            ...
            for (ProtectedRegion test : manager.getRegions().values()) {
                ...
                if (parent != null && parent.equals(region)) {
                    throw new CommandException(
                            "Регион '" + region.getId() + "' имеет дочерние регионы. Используйте -f для принудительного удаления детей " +
                                    "или -u, чтобы отменить родительское значение для этих дочерних элементов.");
                }
            }
