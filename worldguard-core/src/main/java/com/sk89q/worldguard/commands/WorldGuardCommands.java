...
public class WorldGuardCommands {
    ...
    @Command(aliases = {"version"}, desc = "Получить версию WorldGuard", max = 0)
    ...
    @Command(aliases = {"reload"}, desc = "Перезапускает конфигурацию WorldGueard", max = 0)
    ...
    public void reload(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!tasks.isEmpty()) {
            throw new CommandException("В настоящее время есть незавершенные задачи. Сначала используйте /wg running для мониторинга этих задач.");
        }
    ...
    public void report(CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        try {
            ...
            sender.print("WorldGuard отчет записан в " + dest.getAbsolutePath());
        } catch (IOException e) {
    ...
    @Command(aliases = {"profile"}, usage = "[-p] [-i <interval>] [-t <thread filter>] [<minutes>]",
            desc = "Профилируйте загрузку центрального процессора сервера", min = 0, max = 1,
            flags = "t:i:p")
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.argsLength() == 0) {
            ...
            if (minutes < 1) {
                throw new CommandException("Вы должны запустить профиль не менее чем на 1 минуту.");
            } else if (minutes > 10) {
                throw new CommandException("Вы можете создавать профиль максимум в течение 10 минут.");
            }
        }
        ...
        if (args.hasFlag('i')) {
            ...
            if (interval < 10) {
                sender.printDebug("Примечание: Малый интервал может привести к дополнительному замедлению во время профилирования.");
            }
        }
        Sampler sampler;
        ...
        synchronized (this) {
            if (activeSampler != null) {
                throw new CommandException("В настоящее время ведется создание профиля! Пожалуйста, используйте /wg stop profile, чтобы отменить текущий профиль.");
            }
        ...
        sender.print(TextComponent.of("Запуск профилирования процессора. Результаты будут доступны через " + minutes + " минут.", TextColor.LIGHT_PURPLE)
                ...
                .append(TextComponent.of(" в любое время можно отменить профилирование процессора.", TextColor.GRAY)));

        worldGuard.getSupervisor().monitor(FutureForwardingTask.create(
                sampler.getFuture(), "Профилирование процессора займёт " + minutes + " минут", sender));
        ...
        Futures.addCallback(sampler.getFuture(), new FutureCallback<>() {
            ...
            public void onSuccess(Sampler result) {
                ...
                try {
                    ...
                    sender.print("Информация о профилировании процессора записана в " + dest.getAbsolutePath());
                } catch (IOException e) {
                    sender.printError("Не удалость записать информацию о профилировании процессора: " + e.getMessage());
                }
    ...
    public void stopProfile(CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
                throw new CommandException("В данный момент профиль процессора не запущен.");
            }
        ...
        sender.print("Запущенный профиль процессора был отменен.");
    }

    @Command(aliases = {"flushstates", "clearstates"},
            usage = "[player]", desc = "Сбросит управление состоянием", max = 1)
    ...
    public void flushStates(CommandContext args, Actor sender) throws CommandException {
        if (args.argsLength() == 0) {
            ...
            if (player != null) {
                ...
                sender.print("Очищены все состояния для игроков. \"" + player.getName() + "\".");
            }
        }
    }
    ...
    public void listRunningTasks(CommandContext args, Actor sender) throws CommandException {
        ...
        if (tasks.isEmpty()) {
            sender.print("В настоящее время запущенных задач нет.");
        } else {
            tasks.sort(new TaskStateComparator());
            MessageBox builder = new MessageBox("Выполняемые задачи", new TextComponentProducer());
