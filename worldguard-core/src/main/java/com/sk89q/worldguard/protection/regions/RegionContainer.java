...
public abstract class RegionContainer {
    ...
    public void migrate(Migration migration) throws MigrationException {
        ...
        synchronized (lock) {
            try {
                WorldGuard.logger.info("Выгрузка и сохранение загруженных в данный момент данных о регионе...");
                unload();
                migration.migrate();
            } finally {
                WorldGuard.logger.info("Загрузка данных региона для загружаемых слов...");
                loadWorlds();
            }
        }
    }
    ...
    protected void loadWorlds() {
        WorldGuard.logger.info("Загрузка данных о регионе...");
        synchronized (lock) {
    ...
    protected void autoMigrate() {
        ...
        if (config.migrateRegionsToUuid) {
            ...
            try {
                ...
                WorldGuard.logger.info("Регионы, сохраненные после миграции UUID! Этого больше не повторится, если только " +
                        "вы изменяете соответствующий параметр конфигурации в Worldguard");
